...
{"id":17,"first_name":"Heddie","last_name":"Heningam","shirt_size":"L","company_name":"Skinix","donation":172},
...
{"id":50,"first_name":"Shell","last_name":"Baine","shirt_size":"M","company_name":"Gabtype","donation":171}];
console.log(`created runners array`, runners)
...
console.log(`Array of runners using forEach method: `,fullName);

console.log(`==== Challenge 2: Use .map() ====`);
// The event director needs to have all the runner's first names converted to uppercase because the director BECAME DRUNK WITH POWER. Convert each first name into all caps and log the result
console.log(`Uppercase Runners First Names: `,runners.map(runner => runner.first_name.toUpperCase())); 

console.log(`==== Challenge 3: Use .filter() ====`);
// The large shirts won't be available for the event due to an ordering issue.  Get a list of runners with large sized shirts so they can choose a different size. Return an array named largeShirts that contains information about the runners that have a shirt size of L and log the result

console.log(`An array of runners that wear size Large shirts: `,runners.filter(runner => runner.shirt_size === 'L'));

console.log(`==== Challenge 4: Use .reduce() ====`);
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result

console.log(`The sum of all donations: `,`$${runners.reduce((total,runner) =>  total + runner.donation, 0)}`);

console.log(`==== Challenge 5: Be Creative ====`);
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.

// Problem 1 - missing email addresses / modified the array to make a few.
console.log('Runners without emails, using filter(): ',runners.filter(runner => !runner.email));
// Problem 2 - big donators
console.log('Sum of all donations less than $100 using filter and reduce(): ',`$${runners.filter(donation => donation < 100).reduce((x,y) => x + y, 0)}`);
// Problem 3 - someone asks what the highest donation was
console.log('Highest Donation using .map() Math.max():', `$${Math.max(...runners.map(runner => runner.donation))}`);
// Problem 4 - someone asks what the lowest donation was
console.log('Lowest Donation using .map() and Math.min():', `$${Math.min(...runners.map(runner => runner.donation))}`);
// Problem 5 - someone asks what the average donation was
console.log('Average Donation using .reduce() and division operator', `$${runners.reduce((total,runner) => total + runner.donation, 0) / runners.length}`);
// Problem 6 - Return a random runner to win a raffle prize
console.log('Pick a random runner to win a raffle prize', runners[Math.floor(Math.random() * runners.length)]);